...
:url: https://github.com/oblique63/Python-GoogleCalendarParser
"""
...
class CalendarEvent(dict):
    ...
    __slots__ = ( "name", "description", "location", "start_time", "end_time", "all_day",
                  "repeats", "repeat_freq", "repeat_day", "repeat_month", "repeat_until", "uid", "recurrence_id" )
        ...
        for event in self.calendar.walk():
            if isinstance(event, Event):
                ...
                if "RECURRENCE-ID" in event:
                    event_dict["recurrence_id"] = event["RECURRENCE-ID"].to_ical()
                if "UID" in event:
                ...
                if "RRULE" in event:
                    ...
                    if "BYDAY" in rep_dict:
                        event_dict["repeat_day"] = rep_dict["BYDAY"]
